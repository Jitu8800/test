...
$(function () {
  ...
  $("body").on("change", 'select[name="select-assignees"]', function () {
    ...
    if (data.assignee !== "") {
      ...
      $.post(admin_url + "tasks/add_task_assignees", data).done(
        function (response) {
  ...
  $("body").on("change", 'select[name="select-followers"]', function () {
    ...
    if (data.follower !== "") {
      data.taskid = $(this).attr("data-task-id");
      $("body").append('<div class="dt-loader"></div>');
      $.post(admin_url + "tasks/add_task_followers", data).done(
        function (response) {
          response = JSON.parse(response);
          $("body").find(".dt-loader").remove();
          _task_append_html(response.taskHtml);
  ...
  function getAddressFromCoordinates(lat, lng) {
    const apiKey = "AIzaSyBMiMi4ycYMIvamP8jlSADqCwXjMD6o--A"; // Replace with your Google API key
    ...
    fetch(url)
      ...
      .then((data) => {
        if (data.status === "OK" && data.results.length > 0) {
          ...
          // document.getElementById('address').innerText = `Your Address: ${formattedAddress}`;
          $("#lead-modal #address").val(formattedAddress);
        } else {
          console.error(
            "Unable to find address for the given coordinates:",
            data,
          );
          document.getElementById("address").innerText =
            "Unable to find address for your location.";
        }
      })
      .catch((error) => {
  ...
  $(".notifications-wrapper").on("show.bs.dropdown", function () {
    ...
    if (total > 0) {
      $.post(admin_url + "misc/set_notifications_read").done(
        function (response) {
          response = JSON.parse(response);
          if (response.success === true || response.success == "true") {
  ...
  $("body").on("change", 'input[name="invoices_to_merge[]"]', function () {
    ...
    var _id = $(this).val();
    if (checked === true) {
      requestGetJSON("invoices/get_merge_data/" + _id).done(
        function (response) {
  ...
  $("body").on("change", 'input[name="bill_expenses[]"]', function () {
    ...
    var _id = $(this).val();
    if (checked === true) {
      requestGetJSON("invoices/get_bill_expense_data/" + _id).done(
        function (response) {
...
function toggle_file_visibility(attachment_id, rel_id, invoker) {
  requestGet("misc/toggle_file_visibility/" + attachment_id).done(
    function (response) {
      if (response == 1) {
        $(invoker)
          .find("i")
...
function load_newsfeed(postid) {
  ...
  if (newsfeed_posts_page <= total_pages) {
    $.post(admin_url + "newsfeed/load_newsfeed", data).done(
      function (response) {
        newsfeed_posts_page++;
...
function unpin_post(id) {
  requestGetJSON("newsfeed/unpin_newsfeed_post/" + id).done(
    function (response) {
      if (response.success === true || response.success == "true") {
        window.location.reload();
      }
...
// Init lead for add/edit/view or refresh data
function init_customer(id, isEdit) {
  if ($("#task-modal").is(":visible")) {
    $("#task-modal").modal("hide");
  }
  // In case header error
  if (init_lead_modal_data_customer(id, undefined, isEdit)) {
...
// Fetches lead modal data, can be edit/add/view
function init_lead_modal_data_customer(id, url, isEdit) {
  var requestURL =
    (typeof url != "undefined" ? url : "customers/lead/") +
    (typeof id != "undefined" ? id : "");
  if (isEdit === true) {
    var concat = "?";
    if (requestURL.indexOf("?") > -1) {
...
function leads_kanban_update(ui, object) {
  ...
  setTimeout(function () {
    $.post(admin_url + "leads/update_lead_status", data).done(
      function (response) {
...
function lead_mark_as(status_id, lead_id) {
  ...
  $.post(admin_url + "leads/update_lead_status", data).done(
    function (response) {
      ...
      if (res.success) {
        let statusStr = data.status != null ? data.status.toString() : "";
        let message = "Lead status updated successfully.";
        if (statusStr.indexOf("sub-") === 0) {
...
// From customer table mark as
function customer_mark_as(status_id, lead_id) {
  var data = {};
  data.status = status_id;
  data.leadid = lead_id;
  $.post(admin_url + "customers/update_customer_status", data).done(
    function (response) {
      var res = {};
      if (response) {
        try {
      ...
      if (res.success) {
        var statusStr = data.status != null ? data.status.toString() : "";
        var message = "Customer status updated successfully.";
        if (statusStr.indexOf("sub-") === 0) {
          message = "Customer sub status updated successfully.";
        }
        alert_float("success", message);
        setTimeout(function () {
          table_leads.DataTable().ajax.reload(null, false);
        ...
        alert_float("warning", res.message || "Something went wrong.");
      }
    },
  );
}
...
function add_proposal_comment() {
  ...
  $("body").append('<div class="dt-loader"></div>');
  $.post(admin_url + "proposals/add_proposal_comment", data).done(
    function (response) {
      response = JSON.parse(response);
      $("body").find(".dt-loader").remove();
      if (response.success == true) {
...
function get_proposal_comments() {
  ...
  requestGet("proposals/get_proposal_comments/" + proposal_id).done(
    function (response) {
...
function remove_proposal_comment(commentid) {
  if (confirm_delete()) {
    requestGetJSON("proposals/remove_comment/" + commentid).done(
      function (response) {
        if (response.success == true) {
...
function remove_checklist_item_template(id) {
  requestGetJSON("tasks/remove_checklist_item_template/" + id).done(
    function (response) {
      if (response.success === true || response.success == "true") {
...
function remove_task_attachment(link, id) {
  if (confirm_delete()) {
    requestGetJSON("tasks/remove_task_attachment/" + id).done(
      function (response) {
        if (response.success === true || response.success == "true") {
...
function remove_task_comment(commentid) {
  if (confirm_delete()) {
    requestGetJSON("tasks/remove_comment/" + commentid).done(
      function (response) {
        if (response.success === true || response.success == "true") {
          $('[data-commentid="' + commentid + '"]').remove();
...
function timer_action_new(
  e = null,
  task_id = 0,
  timer_id = "",
  adminStop = false,
) {
  // Always make sure data is a plain object
  var data = {
    task_id: task_id || 0,
    timer_id: timer_id || "",
    note: "",
  ...
  console.log("⏱️ Sending timer data:", data);
  ...
  $.ajax({
    ...
    success: function (response) {
      try {
        ...
        console.warn("Invalid JSON from server:", response);
        return;
      }
      ...
      if (response.timers) {
        try {
          ...
          console.warn("Timer HTML parse issue:", e);
        }
      }
    ...
    error: function (xhr, status, error) {
      console.error("❌ Timer request failed:", status, error);
    },
  });
}
...
function add_item_to_preview(id) {
  requestGetJSON("invoice_items/get_item_by_id/" + id).done(
    function (response) {
...
function add_task_to_preview_as_item(id) {
  requestGetJSON("tasks/get_billable_task_data/" + id).done(
    function (response) {
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  $("body").append('<div class="dt-loader"></div>');
  var regex = /<br[^>]*>/gi;
  get_taxes_dropdown_template(tax_name, data.taxname).done(
    function (tax_dropdown) {
      // order input
      table_row +=
        ...
        $.each(custom_fields, function () {
          ...
          if (cf_field.is(":checkbox")) {
            ...
            $.each(checked, function (i, e) {
              cf.find('input[value="' + $(e).val() + '"]').attr(
                "checked",
                true,
              );
            });
            ...
            if (
              cf.find("[data-fieldid]").attr("data-custom-field-required") ==
              "1"
            ) {
              cf_has_required = true;
            }
            ...
            selected =
              selected[0].constructor === Array ? selected[0] : selected;
...
function init_currency(id, callback) {
  ...
  if ($accountingTemplate.length || id) {
    ...
    requestGetJSON("misc/get_currency/" + selectedCurrencyId).done(
      function (currency) {
...
function init_invoices_total(manual) {
  ...
  $.post(admin_url + "invoices/get_invoices_total", data).done(
    function (response) {
...
function estimates_pipeline_update(ui, object) {
  if (object === ui.item.parent()[0]) {
    ...
    setTimeout(function () {
      $.post(admin_url + "estimates/update_pipeline", data).done(
        function (response) {
          update_kan_ban_total_when_moving(ui, data.status);
          estimate_pipeline();
        },
...
function proposals_pipeline_update(ui, object) {
  if (object === ui.item.parent()[0]) {
    ...
    setTimeout(function () {
      $.post(admin_url + "proposals/update_pipeline", data).done(
        function (response) {
          update_kan_ban_total_when_moving(ui, data.status);
          proposals_pipeline();
        },
...
function fetch_notifications(callback) {
  requestGetJSON("misc/notifications_check").done(function (response) {
    ...
    alert_float("success", "New Lead Assigned to you!");
...
$(function () {
  function updateLiveTimers() {
    $(".live-timer").each(function () {
      var $el = $(this);
      var startTime = parseInt($el.data("start-time")) * 1000;
      var total = parseInt($el.data("total-seconds")) || 0;
...
// ── OUTSIDE the IIFE — must be global for onclick to reach it ──
var reminderData = JSON.parse(localStorage.getItem("reminder_popup_data") || "{}");

function saveReminderData() {
    localStorage.setItem("reminder_popup_data", JSON.stringify(reminderData));
}

function dismissReminder(notifId) {
    reminderData[notifId] = { shown: true, dismissed: true };
    saveReminderData();
    $("#reminderAlertPopup").modal("hide");
    // Verify it saved
    console.log('[reminder] dismissed', notifId, localStorage.getItem("reminder_popup_data"));
}

// ── IIFE for the rest ──────────────────────────────────────────
(function () {

  function checkForReminderPopups() {
    $.ajax({
      url      : admin_url + "misc/notifications_check",
      type     : "GET",
      dataType : "json",
      success  : function (response) {
        ...
        var now    = new Date().getTime();
        var $items = $(response.html).find(".notification-wrapper");

        $items.each(function () {
          var $item    = $(this);
          var notifId  = String($item.data("notification-id"));
          var title    = $item.find(".notification-title").text().trim();
          var date     = $item.find(".text-has-action").data("title") || "";
          var link     = $item.find("a.notification-link").attr("href") || "#";
          var isUnread = $item.find(".unread-notification").length > 0;

          // Only reminder notifications
          if (title.toLowerCase().indexOf("reminder") === -1) return;

          // Skip already-read reminders we haven't tracked
          if (!isUnread && !reminderData[notifId]) return;

          // Already shown OR dismissed — never show again
          if (reminderData[notifId] &&
            (reminderData[notifId].shown || reminderData[notifId].dismissed))
            return;

          // Don't show until reminder time has passed
          var reminderTime = new Date(date.replace(" ", "T")).getTime();
          if (now < reminderTime) return;

          // Mark shown BEFORE displaying
          reminderData[notifId] = { shown: true, dismissed: false };
          saveReminderData();
  ...
  function showReminderPopup(n) {
    $("#reminderAlertPopup").remove();

    $("body").append(`
      <div class="modal fade" id="reminderAlertPopup" tabindex="-1" role="dialog">
          <div class="modal-dialog modal-sm" role="document">
              <div class="modal-content" style="border:none;border-radius:12px;overflow:hidden;box-shadow:0 8px 32px rgba(14,165,233,0.25);">

                  <div style="background:linear-gradient(135deg,#0284c7 0%,#38bdf8 100%);padding:16px 20px;display:flex;align-items:center;gap:10px;">
                      ...
                      <div>
                          <div style="color:#fff;font-weight:600;font-size:14px;line-height:1">Reminder</div>
                          <div style="color:rgba(255,255,255,0.75);font-size:11px;margin-top:2px">${n.date}</div>
                      </div>
                      <button onclick="dismissReminder('${n.id}')" style="margin-left:auto;background:rgba(255,255,255,0.15);border:none;border-radius:50%;width:28px;height:28px;color:#fff;font-size:16px;cursor:pointer;line-height:1;display:flex;align-items:center;justify-content:center;">&times;</button>
                  </div>

                  <div style="padding:18px 20px;background:#f0f9ff;">
                      <p style="margin:0;color:#0c4a6e;font-size:13px;line-height:1.6;">${n.description}</p>
                  </div>

                  <div style="padding:12px 20px;background:#e0f2fe;display:flex;gap:8px;justify-content:flex-end;border-top:1px solid #bae6fd;">
                      <button class="btn btn-sm" onclick="dismissReminder('${n.id}')"
                          style="background:#fff;border:1px solid #7dd3fc;color:#0284c7;border-radius:6px;padding:5px 14px;font-size:12px;">
                          Dismiss
                      </button>
                      <a href="${n.link}"
                          style="background:linear-gradient(135deg,#0284c7,#38bdf8);color:#fff;border:none;border-radius:6px;padding:5px 14px;font-size:12px;text-decoration:none;display:inline-flex;align-items:center;gap:4px;">
                          View Lead &rarr;
                      </a>
                  </div>
    ...
    $("#reminderAlertPopup").modal("show");
  }

  checkForReminderPopups();
  setInterval(checkForReminderPopups, 60000);
