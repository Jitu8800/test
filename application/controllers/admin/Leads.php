...
class Leads extends AdminController
{
    ...
    public function index_old($id = '')
    {
        ...
        // Case 1: role empty → keep old behavior (session decides)
        if ($role === '' || $role === null) {
            $leads_kanban_view = $this->session->userdata('leads_kanban_view');

            if ($leads_kanban_view === 'true') {
                $data['isKanBan'] = true;
                $data['switch_kanban'] = false; // false = Kanban
                $data['bodyclass'] = 'kan-ban-body';
            } else {
                $data['isKanBan'] = false;
                $data['switch_kanban'] = true;  // true = List
                unset($data['bodyclass']);
            }
        }
        // Case 2: role = 1,2,3 → always Kanban
        elseif (in_array((int) $role, [1, 2, 3])) {
            $data['isKanBan'] = true;
            $data['switch_kanban'] = false; // force Kanban
            $data['bodyclass'] = 'kan-ban-body';
        }
    ...
    public function lead($id = '')
    {
        ...
        if ($this->input->post()) {
            $dataSource = $this->db->get_where('tblleads_sources', ['id' => $this->input->post('source')])->row();

            $matched_value = '';

            if ($dataSource) {
                $matched_value = $dataSource->value;
            } else {
                $matched_value = '';
            }
            $_POST['lead_value'] = $matched_value;
            $_POST['card_delivery_date'] = date('Y-m-d H:i:s');
            if ($id == '') {
                ...
                if ($id) {
                    ...
                    if ($payment !== null && $payment > 0) {
                        $this->handle_payment_approval($id, $payment, true);
                    }
                }
                ...
                if ($success) {
                    ...
                    $new_payment = $this->input->post('total_payment');
                    if ($new_payment !== null) {
                        $this->handle_payment_approval($id, $new_payment, false);
                    }
    ...
    private function _get_lead_data($id = '')
    {
        ...
        $this->db->select('*');
        $this->db->from(db_prefix().'call_logs');
        $this->db->where('lead_id', $id);
        $this->db->where('staff_id', @$lead->assigned);
        $this->db->order_by('id', 'DESC');
        // $this->db->limit(1);  // ❌ Remove limit if you want all
        $data['latest_call'] = $this->db->get()->result_array(); // ✅ always array
    ...
    {
        if ($this->input->post() && $this->input->is_ajax_request()) {
            $this->leads_model->update_customer_status($this->input->post());
        }
    }
    ...
    public function management($id = '')
    {
        ...
        $data['branches'] = $this->db->get('tblbranches')->result_array();
    ...
    public function make_call()
    {
        ...
        $apiToken = DIALER_API_TOKEN;
        $apiUrl = DIALER_API_URL;
        // $apiToken = config; // Smartflo API Token
        $user = $this->session->userdata();
        $userData = get_staff($user['staff_user_id']);
        $agentNumber = $userData->dialer_agent_id;
        $callerId = $userData->dialer_phone;

        $client = new GuzzleHttp\Client();

        try {
            // 1️⃣ Call Smartflo API
            ...
            $ref_id = $responseBody['ref_id'] ?? null;

            if ($ref_id) {
                // 2️⃣ Save in DB with status QUEUED
                // $this->Call_model->create_call($ref_id, $phone);
                ...
                echo json_encode([
                    'status' => 'danger',
                    'message' => 'Call API responded but no Ref ID returned.',
    ...
    : [];
            ...
            if (!empty($responseBody)) {
                $message = reset($responseBody)[0] ?? $message;
            }
    ...
    // Refund approval related code
    private function handle_payment_approval($lead_id, $amount = 0, $is_create = false)
    {
        if (!$amount || $amount <= 0) {
            return;
        }
        ...
        $total_levels = approval_levels_required('card_sale', (float) $amount);
        ...
        // Prevent duplicates
        $exists = $this->db
            ->where('entity_type', 'card_sale')
            ->where('entity_id', $lead_id)
            ->where('status', 'pending')
            ->count_all_results('tblapproval_requests');

        if ($exists > 0) {
            return;
        }

        $this->Approval_model->create_request([
    ...
    public function export_excel()
    {
        ...
        // Read same filters as DataTable — from GET this time
        $_POST['assigned'] = $this->input->get('assigned') ?: [];
        $_POST['status'] = $this->input->get('status') ?: [];
        $_POST['sub_status'] = $this->input->get('sub_status') ?: [];
        $_POST['source'] = $this->input->get('source') ?: '';
        $_POST['card_status'] = $this->input->get('card_status') ?: '';
        $_POST['payment_mode'] = $this->input->get('payment_mode') ?: '';
        $_POST['date_range'] = $this->input->get('date_range') ?: '';
        $_POST['custom_view'] = $this->input->get('custom_view') ?: '';

        $leads = $this->leads_model->get_leads_for_export();

        // Stream as CSV — never loads all rows into RAM
        ...
        // Headers
        fputcsv($out, [
            'ID', 'Name', 'Phone', 'Email', 'Company', 'Website',
            'Assigned', 'Status', 'Sub Status', 'Source', 'Campaign',
            'City', 'Address', 'Card Status', 'Payment Mode',
            'Last Contact', 'Date Created',
        ]);
