...
class Tasks extends AdminController
{
    ...
    public function init_relation_tasks($rel_id, $rel_type, $sprint='')
    {
        if ($this->input->is_ajax_request()) {
    ...
    public function task($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            $note = '';
        // Fetch hourly rate for the current staff user
        $hourly_rate = $this->db->select('hourly_rate')
            ...
            if ($this->input->post()) {
                ...
                if (is_array($descriptions)) {
                    foreach ($descriptions as $description) {
                        ...
                        if (!$this->tasks_model->add_checklist_item($data)) {
                            $success = false;
                        }
                    }
                } else {
                    ...
                    $success = $this->tasks_model->add_checklist_item($data);
                }
