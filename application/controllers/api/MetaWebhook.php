...
class MetaWebhook extends CI_Controller
{
    ...
    private function saveLead($lead)
    {
        ...
        if (!empty($email) && !empty($phone)) {
            ...
            if ($emailPhoneExists) {
                $this->log('DUPLICATE_LEAD_EMAIL_PHONE', [
                    'meta_lead_id' => $lead['id'],
                    'email' => $email,
                    'phone' => $phone,
                    'existing_id' => $emailPhoneExists->id,
                ]);
            ...
            if ($phoneExists) {
                $this->log('DUPLICATE_LEAD_PHONE', [
                    'meta_lead_id' => $lead['id'],
                    'phone' => $phone,
                    'existing_id' => $phoneExists->id,
                ]);
            ...
            if ($emailExists) {
                $this->log('DUPLICATE_LEAD_EMAIL', [
                    'meta_lead_id' => $lead['id'],
                    'email' => $email,
                    'existing_id' => $emailExists->id,
                ]);
        ...
        $source_id = 3;   // safe default
        $is_on = 1;   // allow by default

        try {
            if (!empty($campaign_id) && !empty($campaign_name)) {
                $sourceResult = $this->upsertSource($campaign_id, $campaign_name);
                $source_id = $sourceResult['id'];
                $is_on = $sourceResult['is_on'];
            }
        } catch (Exception $e) {
            $this->log('SOURCE_UPSERT_FAILED', $e->getMessage());
        }

        // ── is_on = 0 → Campaign is disabled → Skip lead ──
        if ($is_on == 0) {
            $this->log('LEAD_SKIPPED_CAMPAIGN_DISABLED', [
                'meta_lead_id' => $lead['id'],
    ...
    {
        if (empty($campaign_id) || empty($campaign_name)) {
            return ['id' => 3, 'is_on' => 1]; // default fallback — allow
        }
        ...
        if ($existing) {
            ...
            return ['id' => $existing->id, 'is_on' => (int) $existing->is_on];
        }

        // New campaign — insert (default is_on = 1)
