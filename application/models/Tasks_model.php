...
class Tasks_model extends App_Model
{
    ...
    public function add_checklist_item($data)
    {
        ...
        $query = $this->db->get(db_prefix() . 'task_checklist_items');
    ...
    public function timer_tracking($task_id = '', $timer_id = '', $note = '', $adminStop = false)
    {
        ...
        if ($newTimer) {
            ...
            $description = 'Started the Task Timer';
            $this->_send_task_responsible_users_notification($description, $task_id, false, 'task_start_timer_to_staff');
        ...
        if ($timer) {
            ...
            $description = 'Stopped the Task Timer';
            $this->_send_task_responsible_users_notification($description, $task_id, false, 'task_start_timer_to_staff');
            //$this->_send_task_responsible_users_notification($description, $task_id, false, 'task_status_changed_to_staff', serialize($not_data));
        }
    ...
    public function calc_today_logged_time_old($staff_id, $task_id = null)
{
    $today_start = strtotime('today midnight');
    $today_end   = strtotime('tomorrow midnight') - 1;

    $this->db->select('start_time, end_time', false);
    $this->db->from('tbltaskstimers');
    $this->db->where('staff_id', $staff_id);
    $this->db->where('(start_time <= ' . $today_end . ' AND end_time >= ' . $today_start . ')');
    ...
    foreach ($timers as $t) {
        $start = max($today_start, (int)$t->start_time);
        $end   = (int)$t->end_time;
        ...
        $effective_end = min($end, $today_end);

        $total_logged_today += $effective_end - $start;
    }

    return $total_logged_today;
}


public function calc_today_logged_time($staff_id, $task_id = null)
{
    ...
    $this->db->from('tbltaskstimers');
    $this->db->where('staff_id', $staff_id);
    $this->db->where("(start_time <= {$today_end} AND (end_time >= {$today_start} OR end_time IS NULL OR end_time = 0))");
    ...
    foreach ($timers as $t) {
        $start = max($today_start, (int)$t->start_time);
        $end   = (!empty($t->end_time) && $t->end_time > 0) ? (int)$t->end_time : time();
