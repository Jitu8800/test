...
class Authentication_model extends App_Model
{
    ...
    public function login($email, $password, $remember, $staff)
    {
        if ((!empty($email)) and (!empty($password))) {
            ...
            $this->auto_check_in($user->staffid);
    ...
    public function auto_check_in($staff_id)
    {
        ...
        // 1️⃣ Auto close previous day session
        $this->db
        ...
        // 2️⃣ Check if already checked in today
        $existsToday = $this->db
            ->where('staff_id', $staff_id)
            ->where('check_out_time IS NULL', NULL, FALSE)
            ->where('DATE(check_in_time)', $today)
     ...
     * @return none
     */
    public function logout_old($staff = true)
    {
    ...
    public function logout($staff = true)
{
    ...
    if (is_client_logged_in()) {
        ...
        hooks()->do_action('before_staff_logout', $staff_id);
        ...
        $this->dashboard_model->auto_check_out($staff_id);
