...
class Customer_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        $this->db->where('record_type', 'broker');
    ...
    public function add($data)
    {
        ...
        // Public flag
        $data['is_public'] = (isset($data['is_public']) && ($data['is_public'] == 1 || $data['is_public'] === 'on')) ? 1 : 0;

        // Country
        $data['country'] = (!isset($data['country']) || $data['country'] == '') ? 0 : $data['country'];

        // Defaults
        $data['description'] = isset($data['description']) ? nl2br($data['description']) : '';
        $data['dateadded']   = date('Y-m-d H:i:s');
        $data['addedfrom']   = get_staff_user_id();
        $data['record_type'] = 'broker';
        $data['status']      = 1;
        $data['source']      = 1;
        ...
        // Safe string fields
        $data['address']       = isset($data['source_campaign']) ? trim($data['source_campaign']) : '';
        $data['job']           = isset($data['job']) ? trim($data['job']) : '';
        $data['email']         = isset($data['email']) ? trim($data['email']) : '';

        // Safe numeric fields
        $data['payin_amount']  = (isset($data['payin_amount']) && is_numeric($data['payin_amount']))
            ? (float)$data['payin_amount'] : 0.00;
        $data['payout_amount'] = (isset($data['payout_amount']) && is_numeric($data['payout_amount']))
            ? (float)$data['payout_amount'] : 0.00;
     ...
     public function get_status($id = '', $where = [])
{
    ...
    // Cached statuses
    $statuses = $this->app_object_cache->get('customer-all-statuses');
    ...
    if (!$statuses) {
        ...
        $this->app_object_cache->add('customer-all-statuses', $statuses);
    }
...
{
    if (empty($data) || empty($data['leadid']) || !isset($data['status'])) {
        return false;
    }

    // CASE 1: Customer Sub-Status update (incoming value format: "sub-<id>")
    if (is_string($data['status']) && strpos($data['status'], 'sub-') === 0) {
        $subId = (int) str_replace('sub-', '', $data['status']);

        // Get old values
        $this->db->select('customer_status, customer_sub_status');
        $this->db->where('id', $data['leadid']);
        $_old = $this->db->get(db_prefix() . 'leads')->row();

        // Old customer status
        $old_status = '';
        if ($_old && $_old->customer_status) {
            $row = $this->db->get_where(db_prefix() . 'customer_status', ['id' => $_old->customer_status])->row();
            $old_status = $row ? $row->name : '';
        }

        // Old customer sub status
        $old_sub = '';
        if ($_old && $_old->customer_sub_status) {
            $row = $this->db->get_where(db_prefix() . 'customer_substatus', ['id' => $_old->customer_sub_status])->row();
            $old_sub = $row ? $row->sub_name : '';
        }

        // New customer sub status
        $new_sub = '';
        $row = $this->db->get_where(db_prefix() . 'customer_substatus', ['id' => $subId])->row();
        if ($row) {
            $new_sub = $row->sub_name;
        }

        // Update sub status
        $this->db->where('id', $data['leadid']);
        $this->db->update(db_prefix() . 'leads', [
            'customer_sub_status' => $subId,
        ]);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            // ✅ Use customer specific log key
            $this->log_lead_activity($data['leadid'], 'customer_activity_substatus_updated', false, $additional_data);

            log_message('info', 'LeadID: ' . $data['leadid'] . ' | Customer Sub-Status Updated | ' . print_r($additional_data, true));
            echo json_encode([
                'success' => true
            ]);
            return true;
    ...
    // CASE 2: Normal Customer Status update
    $this->db->select('customer_status');
    $this->db->where('id', $data['leadid']);
    $_old = $this->db->get(db_prefix() . 'leads')->row();
    ...
    // New customer status
    $current_status = '';
    $row = $this->db->get_where(db_prefix() . 'customer_status', ['id' => $data['status']])->row();
    if ($row) {
        $current_status = $row->name;
    }
    ...
    if ($this->db->affected_rows() > 0) {
        ...
        if ($current_status != $old_status) {
            ...
            // ✅ Use customer specific log key
            $this->log_lead_activity($data['leadid'], 'customer_activity_status_updated', false, $additional_data);

            hooks()->do_action('customer_status_changed', [
                'lead_id'    => $data['leadid'],
                'old_status' => $old_status,
                'new_status' => $current_status,
            ]);

            log_message('info', 'LeadID: ' . $data['leadid'] . ' | Customer Status Updated | ' . print_r($additional_data, true));
            echo json_encode([
                'success' => true
            ]);
        }
    ...
    // Keep Kanban order update if passed
    if (isset($data['order'])) {
        AbstractKanban::updateOrder($data['order'], 'leadorder', 'leads', $data['status']);
    }
