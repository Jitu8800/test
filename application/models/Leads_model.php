...
class Leads_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        IFNULL('.db_prefix().'leads_status.color, "#000000") AS color,

        '.db_prefix().'leads_substatus.sub_name AS sub_status_name,
        IFNULL('.db_prefix().'leads_substatus.color, "#000000") AS sub_color,

        '.db_prefix().'leads_sources.name AS source_name
    ');

        $this->db->join(db_prefix().'leads_status',
            db_prefix().'leads_status.id = '.db_prefix().'leads.status', 'left');

        $this->db->join(db_prefix().'leads_substatus',
            db_prefix().'leads_substatus.id = '.db_prefix().'leads.sub_status', 'left');

        $this->db->join(db_prefix().'leads_sources',
            db_prefix().'leads_sources.id = '.db_prefix().'leads.source', 'left');
    ...
    public function addApiLead($data)
    {
        if (isset($data['is_public']) && ($data['is_public'] == 1 || $data['is_public'] === 'on')) {
            $data['is_public'] = 1;
        } else {
        ...
        $data['email'] = trim($data['email']);
        $sanitizedString = htmlspecialchars($data['description'], ENT_QUOTES, 'UTF-8');
        $data['lastcontact'] = date('Y-m-d H:i:s');
        $data['description'] = strip_tags($sanitizedString);
        $data['dateadded'] = date('Y-m-d H:i:s');
        $data['addedfrom'] = 1;
        // print_r($data);die;
    ...
    public function leadByMeta($data)
    {
        if (isset($data['is_public']) && ($data['is_public'] == 1 || $data['is_public'] === 'on')) {
            $data['is_public'] = 1;
        } else {
    ...
    public function get_total_leads($statusId)
    {
    ...
    public function get_status($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            if ($status) {
                // Fetch substatuses
                $substatuses = $this->db->where('status_id', $status['id'])
                                        ->get(db_prefix().'leads_substatus')
                                        ->result();

                $status['substatuses'] = $substatuses;
            }
    ...
    public function get_sub_statuses($id = '', $where = [])
    {
        $this->db->where($where);
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get(db_prefix().'leads_substatus')->row_array();
        }

        $this->db->order_by('substatusorder', 'asc');

        return $this->db->get(db_prefix().'leads_substatus')->result_array();
    }

    public function get_customer_status($id = '', $where = [])
    {
        $this->db->where($where);

        if (is_numeric($id)) {
            $this->db->where('id', $id);
            $status = $this->db->get(db_prefix().'customer_status')->row_array();

            if ($status) {
                // Fetch substatuses
                $substatuses = $this->db->where('status_id', $status['id'])
                                        ->get(db_prefix().'customer_substatus')
                                        ->result();
        ...
        if (!$statuses) {
            $this->db->order_by('statusorder', 'asc');
            $statuses = $this->db->get(db_prefix().'customer_status')->result_array();
     ...
     */
    public function add_status_Old($data)
    {
        ...
        $statusData = [
            'name' => $data['name'],
            'color' => $data['color'],
            'statusorder' => isset($data['statusorder']) ? $data['statusorder'] : 0,
            'isdefault' => isset($data['isdefault']) ? $data['isdefault'] : 0,
        ];

        $this->db->insert(db_prefix().'leads_status', $statusData);
        $insert_id = $this->db->insert_id();

        if ($insert_id) {
            // ✅ Handle multiple substatuses
            if (isset($data['sub_name']) && $data['sub_name'] != '') {
                $subNames = explode(',', $data['sub_name']);
                $order = 1;
                ...
                foreach ($subNames as $sub) {
                    ...
                    $subStatusData = [
                        'status_id' => $insert_id, // FK to leads_status
                        'sub_name' => $sub,
                        'color' => $data['color'],
                        'substatusorder' => $order,
                        'isdefault' => isset($data['isdefault']) ? $data['isdefault'] : 0,
                    ];

                    $this->db->insert(db_prefix().'leads_substatus', $subStatusData);
                    ++$order;
                }
            }
    ...
    {
        // Decide table based on type
        $isCustomer = isset($data['customer']) && $data['customer'] == true;

        $status_table = $isCustomer ? 'tblcustomer_status' : db_prefix().'leads_status';
        $substatus_table = $isCustomer ? 'tblcustomer_substatus' : db_prefix().'leads_substatus';

        // Default color
        if (isset($data['color']) && $data['color'] == '') {
            $data['color'] = $isCustomer
                ? hooks()->apply_filters('default_customer_status_color', '#757575')
                : hooks()->apply_filters('default_lead_status_color', '#757575');
        }

        // Default order
        if (!isset($data['statusorder'])) {
            $data['statusorder'] = total_rows($status_table) + 1;
        }
        ...
        // Insert status
        $this->db->insert($status_table, $statusData);
        $insert_id = $this->db->insert_id();
        ...
        if ($insert_id) {
            ...
            if (isset($data['sub_name']) && $data['sub_name'] != '') {
                ...
                foreach ($subNames as $sub) {
                    ...
                    $this->db->insert($substatus_table, $subStatusData);
                    ++$order;
                }
            }
            ...
            log_activity("New {$type} Status Added [StatusID: {$insert_id}, Name: {$data['name']}]");
    ...
    public function update_status($data, $id)
    {
        ...
        $statusData = [
            'name' => $data['name'],
            'color' => isset($data['color']) ? $data['color'] : '#757575',
            'statusorder' => isset($data['statusorder']) ? $data['statusorder'] : 0,
            'isdefault' => isset($data['isdefault']) ? $data['isdefault'] : 0,
        ];

        $this->db->where('id', $id);
        $this->db->update(db_prefix().'leads_status', $statusData);

        $affected = $this->db->affected_rows();

        // ✅ Handle multiple substatuses (replace old with new)
        if (isset($data['sub_name']) && $data['sub_name'] != '') {
            $subNames = explode(',', $data['sub_name']);
            $order = 1;

            // Delete old substatuses for this status
            $this->db->where('status_id', $id);
            $this->db->delete(db_prefix().'leads_substatus');
    ...
    public function update_lead_status($data)
    {
        ...
        $leadAssigned = $this->db->get(db_prefix().'leads')->row();

        if ($leadAssigned && (int) $leadAssigned->assigned === 0) {
            log_message('info', 'LeadID: '.$data['leadid'].' | Status update blocked | Lead not assigned');

            echo json_encode([
                'success' => false,
                'message' => 'Please assign this lead first before changing status.',
            ]);
            exit;
        }

        // CASE 1: Sub-Status update
        if (is_string($data['status']) && strpos($data['status'], 'sub-') === 0) {
            $subId = (int) str_replace('sub-', '', $data['status']);

            // Get old status + sub status
            $this->db->select('status, sub_status');
            $this->db->where('id', $data['leadid']);
            $_old = $this->db->get(db_prefix().'leads')->row();

            // Get old parent status
            $old_status = '';
            if ($_old && $_old->status) {
                $old_status_obj = $this->get_status($_old->status);
                if ($old_status_obj) {
                    $old_status = is_array($old_status_obj) ? $old_status_obj['name'] : $old_status_obj->name;
                }
            }

            // Get old sub status
            $old_sub = '';
            if ($_old && $_old->sub_status) {
                $old_sub_obj = $this->get_sub_status($_old->sub_status);
                if ($old_sub_obj) {
                    $old_sub = is_array($old_sub_obj) ? $old_sub_obj['sub_name'] : $old_sub_obj->sub_name;
                }
            }

            // Get new sub status
            $new_sub_obj = $this->get_sub_status($subId);
            $new_sub = $new_sub_obj
                ? (is_array($new_sub_obj) ? $new_sub_obj['sub_name'] : $new_sub_obj->sub_name)
                : '';

            // Update lead with new sub status
            $this->db->where('id', $data['leadid']);
            $this->db->update(db_prefix().'leads', [
                'sub_status' => $subId,
                'last_sub_status_change' => date('Y-m-d H:i:s'),
            ]);

            if ($this->db->affected_rows() > 0) {
                // Save in DB activity log
                $additional_data = serialize([
                    get_staff_full_name(),
                    $old_status ?: 'N/A',
                    $old_sub ?: 'N/A',
                ...
                $this->log_lead_activity($data['leadid'], 'lead_sub_status_updated', false, $additional_data);

                log_message('info', 'LeadID: '.$data['leadid'].' | Sub Status Updated | '.print_r($additional_data, true));
                echo json_encode([
                    'success' => true,
                ]);
        ...
        $affectedRows = 0;
        $current_status_obj = $this->get_status($data['status']);
        $current_status = $current_status_obj
            ? (is_array($current_status_obj) ? $current_status_obj['name'] : $current_status_obj->name)
            : '';
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($data['status'] == 8) {
                $updateData['record_type'] = 'broker';
                $updateData['customer_status'] = 1;
            } else {
                $updateData['record_type'] = 'lead';
                $updateData['customer_status'] = null;
            }

            $this->db->where('id', $data['leadid']);
            $this->db->update(db_prefix().'leads', $updateData);
        }
        ...
        if ($affectedRows > 0) {
            ...
            // Save also in log file
            log_message('info', 'LeadID: '.$data['leadid'].' | Status Updated | '.print_r($additional_data, true));
            echo json_encode([
                'success' => true,
            ]);
     ...
     */
    public function get_sub_status($id)
    {
        if (!$id) {
            return null;
        }
    ...
    public function get_customer_sub_status($id)
    {
        if (!$id) {
            return null;
        }
    ...
    public function get_next_lead($staff_id)
    {
        $this->db->where('assigned', $staff_id);
        $this->db->where('status =', 4); // new leads only
    ...
    public function mark_lead_answered($lead_id)
    {
        $this->db->where('id', $lead_id);
    ...
    public function save_call_log($data)
    {
        return $this->db->insert('tblcall_logs', $data);
    }

 public function get_leads_for_export()
{
    $CI    = &get_instance();
    $where = [];
    ...
    if ($postSubStatus && is_array($postSubStatus)) {
        ...
        $where[] = 'AND l.sub_status IN(' . $substatusesEscaped . ')';
    }

    if ($CI->input->post('source')) {
        $where[] = 'AND l.source = ' . (int) $CI->input->post('source');
    }

    if ($CI->input->post('card_status')) {
        $where[] = 'AND l.card_status = "' . $CI->db->escape_str($CI->input->post('card_status')) . '"';
    }

    if ($CI->input->post('payment_mode')) {
        $where[] = 'AND l.payment_mode = "' . $CI->db->escape_str($CI->input->post('payment_mode')) . '"';
    }
    ...
    if (!has_permission('leads', '', 'view')) {
        ...
        $where[] = 'AND (l.assigned = ' . $userId
            . ' OR l.addedfrom = ' . $userId
            . ' OR l.is_public = 1)';
    }
    ...
    if ($custom_view === 'lost') {
        $where[] = 'AND l.lost = 1';
    } elseif ($custom_view === 'junk') {
        $where[] = 'AND l.junk = 1';
    } elseif ($custom_view === 'contacted_today') {
        $where[] = 'AND DATE(l.lastcontact) = CURDATE()';
    } elseif ($custom_view === 'created_today') {
        $where[] = 'AND DATE(l.dateadded) = CURDATE()';
    }

    $sql = 'SELECT
                l.id, l.name, l.phonenumber, l.email, l.company,
                l.website, l.city, l.address, l.source_campaign,
                l.card_status, l.payment_mode, l.lastcontact, l.dateadded,
                ls.name      AS status_name,
                ss.sub_name  AS sub_status_name,
            ...
            LEFT JOIN ' . db_prefix() . 'leads_substatus ss  ON ss.id     = l.sub_status
            LEFT JOIN ' . db_prefix() . 'leads_sources src   ON src.id    = l.source
