...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            // ==================================================
            // ✅ ADD AUTOMATED REMINDERS HERE
            // ==================================================
            // $this->load->model('automated_reminders_model');
            // $this->automated_reminders_model->run_all();
            $this->load->model('Daily_admin_summary_model');
            $this->Daily_admin_summary_model->send();
    ...
    private function tasks_reminders()
    {
        ...
        foreach ($tasks as $task) {
            //  print_r(date('Y-m-d', strtotime($task['duedate'])));
            //  echo '===='.date('Y-m-d'). "<br>";
            if (date('Y-m-d', strtotime($task['duedate'])) >= date('Y-m-d')) 
            {
              ...
              //  echo $diff.'<='.$reminder_before.'&&'.$start_and_due_date_diff.'>'.$reminder_before;die;
                ...
                if ($diff <= $reminder_before && $start_and_due_date_diff > $reminder_before) 
                {
                    ...
                    foreach ($assignees as $member) 
                    {
                        ...
                        { 
                           ...
                           $ss = send_mail_template('task_deadline_reminder_to_staff', $row->email, $member['assigneeid'], $task['id']);
...
//Added By anupam manually ------------------------------------------------------------------------------------------------
                        $teamLeadArr = [
                                            'aman@metaspacechain.com', 
                                            'piyush@metaspacechain.com',
                                            'shekhar@metaspacechain.com',
                                            'priya@metaspacechain.com',
                                            'barkha@bastionex.net', 
                                            'ajay@metaspacechain.com', 
                                            'durgesh@bastionex.net'
                                        ];
                        $this->db->select('email, staffid');
                        $this->db->where_in('email', $teamLeadArr);
                        $rowss = $this->db->get(db_prefix() . 'staff')->result_array();
                        if ($rowss) 
                        {
                            foreach ($rowss as $rowOne) 
                            {
                                $notified = add_notification([
                                                                'description'     => 'not_task_deadline_reminder',
                                                                'touserid'        => $rowOne['staffid'],
                                                                'fromcompany'     => 1,
                                                                'fromuserid'      => 0,
                                                                'link'            => '#taskid=' . $task['id'],
                                ...
                                if ($notified) {
                                    array_push($notifiedUsers, $rowOne['staffid']);
                                }

                                send_mail_template('task_deadline_reminder_to_staff', $rowOne['email'], $rowOne['staffid'], $task['id']);
