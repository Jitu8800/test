...
class Branch_model extends App_Model
{
    ...
    {
        $this->db->select('*,' . db_prefix() . 'leads.name, ' . db_prefix() . 'leads.id,' . db_prefix() . 'leads_status.name as status_name,' . db_prefix() . 'branches.name as source_name');
        $this->db->join(db_prefix() . 'leads_status', db_prefix() . 'leads_status.id=' . db_prefix() . 'leads.status', 'left');
        $this->db->join(db_prefix() . 'branches', db_prefix() . 'branches.id=' . db_prefix() . 'leads.source', 'left');
    ...
    public function add($data)
    {
        ...
        $data['address'] = trim($data['source_campaign']) ?? '';
        $data['job'] = trim($data['job']) ?? '';
        $data['assigned']  = (isset($data['assigned']) && is_numeric($data['assigned']))
            ? (int)$data['assigned'] : 0;
...
{
    $current = $this->get_source($id); // optional, if you need the current branch details

    // ✅ Check if this branch is already assigned to any staff
    if (is_reference_in_table('branch_id', db_prefix() . 'staff', $id)) {
        return [
            'referenced' => true,
        ];
    }
    ...
    if ($this->db->affected_rows() > 0) {
        log_activity('Branch Deleted [BranchID: ' . $id . ']');
        return true;
    }
