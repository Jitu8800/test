    ...
    public function downloadSample()
{
    ...
    foreach ($this->getImportableDatabaseFields() as $field) {
        if (in_array(strtolower($field), $this->getHiddenFields())) {
            continue;
        }
        $allFields[] = $field;
    ...
    for ($row = 0; $row < $totalSampleRows; $row++) {
        foreach ($allFields as $field) {
            $sampleDataText = $this->getTableRowDataText($field);
            echo '"' . $sampleDataText . '",';
        }
...
public function createSampleTableHtml($simulation = false)
{
    ...
    {
        return ['record_type', 'title', 'company', 'description', 'zip', 'tags'];
    }
