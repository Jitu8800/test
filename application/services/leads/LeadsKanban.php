...
class LeadsKanban extends AbstractKanban
{
    ...
    {
        if (!startsWith($q, '#')) {
            if ($this->getLastSegment() === 'customers') {
        $this->ci->db->where(db_prefix() . 'leads.record_type', 'broker');
    }
    ...
    protected function initiateQuery()
    {
        ...
        /**
     * Get last segment of current URL (fallback to '')
     */
    private function getLastSegment()
    {
