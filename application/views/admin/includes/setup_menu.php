...
<div id="setup-menu-wrapper" class="sidebar animated <?php if ($this->session->has_userdata('setup-menu-open')
    ...
    <ul class="nav metis-menu" id="setup-menu">
        ...
        // echo "<pre>"; print_r($setup_menu); echo "</pre>";
        foreach ($setup_menu as $key => $item) {
              if (in_array($key, ['finance', 'support', 'contracts','knowledge-base','subscriptions','expenses','sales','projects','tasks','estimate_request','utilities','modules'])) {
                continue;
            }
