...
$aColumns = array_merge($aColumns, ['company',
    ...
    'lead_value', 'client_business_link', 'card_ordered', 'card_status', 'card_delivered', 'card_delivery_date', 'payment_mode', 'total_payment', 'payment_due', 'payment_discount', 'address',
    ...
    '(SELECT long_name FROM ' . db_prefix() . 'countries WHERE ' . db_prefix() . 'countries.country_id = ' . db_prefix() . 'leads.country LIMIT 1) as country',
...
if($userRoles->role == 3){
    ...
    $where = ['AND ' . db_prefix() . 'leads.assigned ='.$userRoles->staffid];
}
...
if ($this->ci->input->post('card_status')) {    
    array_push($where, "AND card_status ='" . $this->ci->db->escape_str($this->ci->input->post('card_status')) ."'") ;
}


if ($this->ci->input->post('payment_mode')) {    
    array_push($where, "AND payment_mode ='" . $this->ci->db->escape_str($this->ci->input->post('payment_mode')) ."'") ;
}
...
foreach ($rResult as $aRow) {
    ...
    $row[] = $aRow['client_business_link'];
    $row[] = $aRow['card_ordered'];
    $row[] = $aRow['card_status'];
    $row[] = $aRow['card_delivered'];
    $row[] = $aRow['card_delivery_date'];
    $row[] = $aRow['payment_mode'];
    $row[] = $aRow['total_payment'];
    $row[] = $aRow['payment_due'];
    $row[] = $aRow['payment_discount'];
    $row[] = $aRow['address'];
