...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); ++$i) {
        ...
        } elseif ($aColumns[$i] == 'auto_assign_leads') {
            $checked = $aRow['auto_assign_leads'] == '1' ? 'checked' : '';

            $_data = '<div class="onoffswitch">
        <input type="checkbox" 
            class="onoffswitch-checkbox auto-assign-toggle"
            id="auto_'.$aRow['staffid'].'"
            data-id="'.$aRow['staffid'].'"
            '.$checked.'>
        <label class="onoffswitch-label" for="auto_'.$aRow['staffid'].'"></label>
    </div>';

            // Export fix
            $_data .= '<span class="hide">'.($checked ? 'Enabled' : 'Disabled').'</span>';
