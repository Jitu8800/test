...
$total_logged = (int) $this->tasks_model->calc_task_total_time($timer['task_id'], ' AND staff_id=' . get_staff_user_id());
$today_seconds = (int) $this->tasks_model->calc_today_logged_time(get_staff_user_id(), $timer['task_id']);

// If timer still running, live-update both total and today
$is_running = empty($timer['end_time']);

// If running, subtract the current running elapsed from $today_seconds so JS will add live elapsed
if ($is_running) {
    $running_elapsed = time() - $start_time; // seconds
    $today_seconds_static = max(0, $today_seconds - $running_elapsed);
    $total_logged_static = max(0, $total_logged - $running_elapsed);
} else {
    $today_seconds_static = $today_seconds;
    $total_logged_static = $total_logged;
}

// $data .= '
// <span class="tw-text-neutral-600">' . _l('timer_top_started', _dt($start_time, true)) . '</span><br />

// <span class="text-success tw-text-sm">
//   ' . _l('task_total_logged_time') . ' 
//   <span class="live-timer total-logged"
//         data-start-time="' . ($is_running ? $start_time : 0) . '"
//         data-total-seconds="' . $total_logged_static . '">
//         ' . seconds_to_time_format($total_logged) . '
//   </span>
// </span><br />

// <span class="text-info tw-text-sm">
//   ' . _l('Today Logged Time') . ': 
//   <span class="live-timer today-logged"
//         data-start-time="' . ($is_running ? $start_time : 0) . '"
//         data-total-seconds="' . $today_seconds_static . '">
//         ' . seconds_to_time_format($today_seconds) . '
//   </span>
// </span>
// ';
