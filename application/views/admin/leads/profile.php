...
<div class="<?php if ($openEdit == true) {
    ...
    <div id="modalDialerStatus">
        <div id="callTimerBox"><b>⏱️ 00:00</b></div>
        <div id="callMessageBox"></div>
    </div>
        ...
        <div class="lead-view<?php if (!isset($lead)) {
          echo ' hide';
            ...
            <div class="col-md-4 col-xs-12 lead-information-col">
                <div class="lead-info-heading">
                    <h4>
                        <?php echo _l('Call Recordings'); ?>
                    </h4>
                </div>


                <?php if (!empty($latest_call)) : ?>
                <div id="recordingList">
                    <?php foreach ($latest_call as $rec) : ?>
                    <?php if (!empty($rec['recording_url'])) : ?>
                    <div style="padding:5px 0;">
                        <a href="javascript:void(0)" onclick="playRecording('<?= $rec['recording_url'] ?>')">
                            ▶
                            Status: <?= $rec['status'] ?: 'N/A' ?>,
                            Call Status: <?= $rec['call_status'] ?: 'N/A' ?>,
                            Agent: <?= $rec['answered_agent_name'] ?: 'N/A' ?>
                            (<?= $rec['duration'] ?: 0 ?> sec)
                        </a>
                    </div>
                    <?php endif; ?>
                ...
                <?php else : ?>
                <div id="recordingList">
                    <em>No recordings available</em>
                </div>
                <?php endif; ?>

                <div id="audioPlayerBox" style="display:none; margin-top:10px;">
        ...
        <div class="lead-edit<?php if (isset($lead)) {
          ...
          ?>
            ...
            $value = (isset($lead) ? $lead->phonenumber : ''); ?>
                ...
                <!-- new leads input added--------- -->
                <?php $value = (isset($lead) ? $lead->client_business_link : ''); ?>
                <?php echo render_input('client_business_link', 'Client Business Link', $value); ?>
                <?php $value = (isset($lead) ? $lead->card_ordered : ''); ?>
                <?php echo render_input('card_ordered', 'Card Ordered', $value); ?>
                ...
                $selected = isset($lead) ? $lead->card_status : '';
                echo render_select(
                    'card_status',
                    $card_status,
                    ['id', 'label'],
                ...
                $selected = isset($lead) ? $lead->card_delivered : '';
                echo render_select(
                    'card_delivered',
                    $card_delivered,
                    ['id', 'label'],
            ...
            <div class="col-md-6">
               ...
               ?>
                ...
                <div class="lead-select-date-contacted">
                    <?php echo render_datetime_input('card_delivery_date', 'Card Delivery Date', '', ['data-date-end-date' => date('Y-m-d')]); ?>
                </div>
                <?php } else {   ?>
                <?php echo render_datetime_input('card_delivery_date', 'Card Delivery Date', _dt($lead->card_delivery_date), ['data-date-end-date' => date('Y-m-d')]); ?>
                <?php } ?>
                ...
                $selected = isset($lead) ? $lead->payment_mode : '';
                echo render_select(
                    'payment_mode',
                    $payment_mode,
                    ['id', 'label'],
                ...
                <?php $value = (isset($lead) ? $lead->total_payment : ''); ?>
                <?php echo render_input('total_payment', 'Total Payment', $value, $type = 'text', $input_attrs = ["onkeypress"=>"return isNumberKey(event, this)"]); ?>
                <?php $value = (isset($lead) ? $lead->payment_due : ''); ?>
                <?php echo render_input('payment_due', 'Payment Due', $value, $type = 'text', $input_attrs = ["onkeypress"=>"return isNumberKey(event, this)"]); ?>
                <?php $value = (isset($lead) ? $lead->payment_discount : ''); ?>
                <?php echo render_input('payment_discount', 'Payment Discount', $value, $type = 'text', $input_attrs = ["onkeypress"=>"return isNumberKey(event, this)"]); ?>
            </div>
